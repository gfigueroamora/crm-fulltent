...
import {
  ESTADOS, PIPE_STAGES, MESES_KEYS, MESES_LABELS, RUBRO_LIST,
  ESTADO_CFG, PIE_COLORS, FTBLUE
} from '../constants';
import { fmtCLP, fmtM2, fmtDate, parseExcelFile } from '../utils';
import Modal from './Modal';

const COLLECTION = 'solicitudes';

function Badge({ estado }) {
  const cfg = ESTADO_CFG[estado] || ESTADO_CFG['Sin estado'];
  return (
    <span style={{
      background: cfg.bg, color: cfg.c, padding: '2px 8px',
...
function Toast({ msg, type }) {
  ...
  return msg ? (
    <div style={{
      ...
      background: cols[type] || '#16a34a', color: '#fff',
      padding: '10px 18px', borderRadius: 9, fontSize: 12,
      fontWeight: 600, boxShadow: '0 4px 20px rgba(0,0,0,.25)'
    }}>{msg}</div>
  ) : null;
}
...
export default function CRM({ user }) {
  ...
  // Export
  const exportCSV = () => {
    const cols = ['empresa','nombre','rubro','producto','medidas','m2','tipo',
      'mail','telefono','contacto','nCot','pCierre','monto','estado',
      'dia','mes','anio','fecha','observacion'];
  ...
  // Filters & sorting
  const allMeses  = useMemo(() => [...new Set(records.map(r => r.mes).filter(Boolean))], [records]);
  ...
  const rubroList = useMemo(() => {
    const extra = allRubros.filter(r => !RUBRO_LIST.includes(r));
    return [...RUBRO_LIST, ...extra];
  }, [allRubros]);
  ...
  const tasaConv  = ganados.length + perdidos.length > 0
    ? Math.round(ganados.length / (ganados.length + perdidos.length) * 100) : 0;
  ...
  const onDrop = (e, stage) => {
    e.preventDefault();
    if (dragId) updateEstado(dragId, stage);
    setDragId(null);
  };
  ...
  const Th = ({ col, label }) => (
    <th onClick={() => doSort(col)} style={{
      ...
      fontSize: 11, background: '#f8fafc', borderBottom: '2px solid #e5e7eb',
  ...
  if (loading) return (
    <div style={{ ...ss, display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
      <div style={{ textAlign: 'center', color: '#6b7280' }}>
        ...
        <div style={{ fontWeight: 600 }}>Cargando CRM Fulltent...</div>
      </div>
    </div>
  );
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: '#fff', borderRadius: 12, padding: 26, width: 320, textAlign: 'center', boxShadow: '0 8px 32px rgba(0,0,0,.2)' }}>
            ...
            <div style={{ color: '#6b7280', fontSize: 12, marginBottom: 18 }}>
              Se eliminará <b>{delTarget.empresa}</b>. No se puede deshacer.
            </div>
        ...
        <div style={{ display: 'flex', gap: 5 }}>
          {[['lista','📋 Lista'],['pipeline','📊 Pipeline'],['stats','📈 Estadísticas']].map(([v, lbl]) => (
            <button key={v} onClick={() => setView(v)} style={{
              ...
              background: view === v ? 'rgba(255,255,255,.25)' : 'rgba(255,255,255,.1)',
              color: '#fff', borderBottom: view === v ? '2px solid #fff' : '2px solid transparent'
            }}>{lbl}</button>
          ))}
        </div>
      ...
      {view !== 'stats' && (
        <div style={{ padding: '0 16px 12px', display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          <input value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
            placeholder="🔍 Empresa, nombre, N° cotización..."
          ...
          <select value={fMes} onChange={e => { setFMes(e.target.value); setPage(1); }} style={sel}>
            <option value="">Todos los meses</option>
            {allMeses.map(m => <option key={m}>{m}</option>)}
          </select>
          <select value={fEstado} onChange={e => { setFEstado(e.target.value); setPage(1); }} style={sel}>
          ...
          {hasFilter && (
            <button onClick={clearFilters} style={{ padding: '6px 11px', borderRadius: 6, border: 'none', background: '#fee2e2', color: '#dc2626', fontWeight: 700, cursor: 'pointer', fontSize: 12 }}>
              ✕ Limpiar
            </button>
          )}
        </div>
        ...
        <div style={{ padding: '0 16px 16px', overflowX: 'auto' }}>
          <div style={{ display: 'flex', gap: 8, minWidth: 1100 }}>
            {PIPE_STAGES.map(stage => {
              ...
              return (
                <div key={stage} style={{ flex: 1, minWidth: 118 }}
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 14, borderBottom: '2px solid #e5e7eb' }}>
            {[['resumen','📊 Resumen'],['distribucion','🏭 Distribución'],['tendencia','📈 Tendencia']].map(([t, lbl]) => (
              <button key={t} onClick={() => setStatsTab(t)} style={{
                ...
                fontSize: 12, fontWeight: 600,
                color: statsTab === t ? FTBLUE : '#6b7280',
                borderBottom: statsTab === t ? `2px solid ${FTBLUE}` : '2px solid transparent',
                marginBottom: -2
              }}>{lbl}</button>
            ))}
          </div>
          ...
          {statsTab === 'distribucion' && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div style={{ background: '#fff', borderRadius: 10, padding: '16px 18px', boxShadow: '0 1px 4px rgba(0,0,0,.07)' }}>
                ...
                {byEstado.map(d => (
                  <div key={d.label} onClick={() => { setFEstado(d.label); setView('lista'); }}
                ...
                {forecast.map(f => (
                  <div key={f.stage} style={{ marginBottom: 10 }}>
                    ...
                    <div style={{ background: '#f3f4f6', borderRadius: 99, height: 6 }}>
                      <div style={{ width: Math.round(f.pond / (forecast[0]?.pond || 1) * 100) + '%', height: 6, borderRadius: 99, background: f.cfg.c }} />
                    </div>
                  </div>
                ))}
      ...
      {detail && (
        <div style={{ position: 'fixed', right: 0, top: 0, bottom: 0, width: 355, background: '#fff', boxShadow: '-4px 0 24px rgba(0,0,0,.14)', zIndex: 100, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ padding: 16, flex: 1 }}>
            ...
            {detail.observacion && (
              <div style={{ marginTop: 10, background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 8, padding: 10 }}>
