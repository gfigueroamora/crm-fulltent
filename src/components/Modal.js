...
const EMPTY = {
  ...
  pCierre:'', monto:'', fecha:'', mes:'', estado:'NUEVA SOLICITUD',
  observacion:'', comentarioSeg:'', fechaSeg:'', fechaProxSeg:''
};
...
export default function Modal({ rec, onSave, onClose, rubroList }) {
  ...
  return (
    <div style={{ position:'fixed',inset:0,background:'rgba(0,0,0,.55)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center' }}>
      <div style={{ background:'#fff',borderRadius:13,width:580,maxHeight:'92vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,.3)' }}>
        <div style={{ background:FTBLUE,color:'#fff',padding:'14px 18px',borderRadius:'13px 13px 0 0',display:'flex',justifyContent:'space-between',alignItems:'center' }}>
          <span style={{ fontWeight:700,fontSize:14,color:'#fff' }}>{rec?.id ? '✏️ Editar registro' : '➕ Nuevo prospecto'}</span>
        ...
        <div style={{ padding:18 }}>
          ...
          <Row><F label="N° Cotización" k="nCot"/><F label="% Cierre (0–100)" k="pCierre" type="number"/></Row>
          <Row><F label="Monto CLP" k="monto" type="number"/><F label="Fecha" k="fecha" type="date"/></Row>
          <Row>
            <S label="Mes" k="mes" opts={['','ENE','FEB','MAR','ABR','MAY','JUN','JUL','AGO','SEP','OCT','NOV','DIC']}/>
            <div><label style={lbl}>Estado</label>
              <select value={form.estado} onChange={e=>set('estado',e.target.value)}
                style={{...inp,borderLeft:`4px solid ${(ESTADO_CFG[form.estado]||{c:'#6b7280'}).c}`}}>
                {ESTADOS.map(e=><option key={e}>{e}</option>)}
              </select>
            </div>
          </Row>
          <Row>
            <F label="Fecha seguimiento" k="fechaSeg" type="date"/>
            <F label="Próximo seguimiento" k="fechaProxSeg" type="date"/>
          </Row>
          <div style={{ marginBottom:10 }}>
            <label style={lbl}>Comentarios seguimiento</label>
            <textarea value={form.comentarioSeg} onChange={e=>set('comentarioSeg',e.target.value)}
              rows={2} style={{...inp,resize:'vertical'}}
              placeholder="Notas del seguimiento comercial..."/>
          </div>
          <div style={{ marginBottom:16 }}>
            <label style={lbl}>Observación general</label>
